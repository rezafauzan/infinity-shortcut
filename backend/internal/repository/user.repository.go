...
func (u UserRepository) CreateNewUser(newUser models.User) (models.User, error) {
	sql := "INSERT INTO users (name, email, password, created_at, updated_at) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, email, password, created_at, updated_at"
	rows, err := u.db.Query(context.Background(), sql, newUser.Name, newUser.Email, newUser.Password, time.Now(), time.Now())
	if err != nil {
